...
class JobboleSpider(scrapy.Spider):
    ...
    def parse_article_data(self,response):
        ...
        # # 评论量
        articleItem['commentNum'] = response.xpath(
            '//span[@class="btn-bluet-bigger href-style hide-on-480"]/text()').extract_first()
        # # 收藏
        articleItem['markNum'] = response.xpath('//span[@class=" btn-bluet-bigger href-style bookmark-btn  register-user-only "]/text()').extract_first('0')
        # # 发布时间
        articleItem['publishTime'] = ','.join(response.xpath('//p[@class="entry-meta-hide-on-mobile"]/text()').extract()).replace(' ','').replace('\n','').replace('\r','')
